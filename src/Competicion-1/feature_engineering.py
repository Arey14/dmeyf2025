...
if __name__ == "__main__":
    ...
    if run_step2:
        if not step2_engineer_features(): exit()
        
    if run_step2b:
        # Se asegura de que el paso 2 se haya ejecutado si su archivo de salida no existe
        if not os.path.exists(os.path.join(DATA_PATH, FEATURE_ENGINEERED_FILE)):
            print("Ejecutando paso 2 antes del paso 2b...")
            if not step2_engineer_features(): exit()
        if not step2b_add_rf_features(): exit()
