...
# (Métrica de ganancia personalizada eliminada)
# ---------------------------------

# --- 2. Métrica de Ganancia Personalizada ---
# (Función 'lgbm_profit_metric' eliminada)
# ---------------------------------
...
try:
    ...
    for seed in N_SEEDS:
        ...
        lgb_model = lgb.LGBMClassifier(
            objective='binary',
            metric='auc', # <-- MODIFICADO (antes 'none')
            n_estimators=1000,
            learning_rate=0.05,
            n_jobs=-1,
        ...
        lgb_model.fit(
            ...
            eval_metric='auc', # <-- MODIFICADO (antes lgbm_profit_metric)
            callbacks=[lgb.early_stopping(100, verbose=False)],
            categorical_feature=cat_features_final
        )
        
        # --- MODIFICADO: Reportar AUC en lugar de Ganancia ---
        best_auc = lgb_model.best_score_['valid_0']['auc']
        print(f"  Mejor AUC (Seed {seed}): {best_auc:,.4f}")
